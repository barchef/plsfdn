...
func Serve(addr string) {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		// Serve the embedded PulseChain Explorer files
		http.FileServer(http.FS(subFS)).ServeHTTP(w, r)
	})

	ready := make(chan struct{})
	logrus.Info("Starting PulseChain Explorer server")
	...
	// Wait for startup
	<-ready
	logrus.Infof("You can access the PulseChain Explorer server at: %s", url)
